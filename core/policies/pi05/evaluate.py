...
def _run_task_graph_eval(args: EvalArgs, policy, preprocessor, postprocessor, task: str) -> None:
    ...
    for ep_idx in pbar:
        ...
        for _ in range(int(target_steps)):
            ...
            # Stage flags are running maxes: once a predicate fires we keep
            # it set even if it transiently flips off later (e.g. release).
            try:
    ...
    print(f"\n{'='*48}")
    print(f"PI0.5 task-graph eval: {args.task_graph}")
