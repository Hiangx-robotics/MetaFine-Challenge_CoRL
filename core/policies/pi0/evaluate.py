"""PI0 policy evaluation on FGManip/ManiSkill environments.

使用与 lerobot 官方 lerobot_eval.py 一致的方式加载 checkpoint（模型 + preprocessor +
postprocessor），然后在 FGManip 自定义环境中运行 rollout 并统计成功率。

Usage:
    CUDA_VISIBLE_DEVICES=0 python FGManip/core/policies/pi0/evaluate.py \
...
# =============================================================================
# 不覆盖 CUDA_VISIBLE_DEVICES：由 shell（如 CUDA_VISIBLE_DEVICES=0）决定用哪张卡。
...
def _setup_paths() -> None:
    ...
    否则与同级的 pi05 目录共用其 vendored Lerobot，再回退到项目根目录的 lerobot/src。
    """
    ...
    if local_lerobot_src.exists() and local_lerobot_src.is_dir():
        ...
        sibling_lerobot = script_dir.parent / "pi05" / "Lerobot" / "src"
        if sibling_lerobot.exists() and sibling_lerobot.is_dir():
            lerobot_path = str(sibling_lerobot)
            if lerobot_path not in sys.path:
                sys.path.insert(0, lerobot_path)
        else:
...
class EvalArgs:
    ...
    # New: task-graph mode. When set, the env is built via
    # utils.eval_setup.make_eval_env using the YAML at this path (multi_skill
    # env with goal_predicate + optional stage predicates). All single-skill
    # CLI flags above are ignored in that case.
    task_graph: str | None = None
    # New: when set, write the new-style EvalSummary JSON to this path on top
    # of the existing metrics_summary.json. Defaults to <record_dir>/<env>/results.json.
    results_json: str | None = None


def parse_args() -> EvalArgs:
    p = argparse.ArgumentParser(description="Evaluate PI0 on FGManip envs")
    ...
    p.add_argument("--target-letter", default=None,
        help="target_letter kwarg for assembling_kits_metafine_letter env (M/E/T/A/F/I/N).")
    ...
    p.add_argument("--task-graph", default=None,
                   help="Path to a task-graph YAML. When set, the env is built "
                        "via utils.eval_setup.make_eval_env (multi_skill + "
                        "goal_predicate + optional stages); --env-id/--object-name "
                        "/--part-name are ignored.")
    p.add_argument("--results-json", default=None,
                   help="Where to write the new EvalSummary JSON. "
                        "Defaults to <record_dir>/<env>/results.json.")
    ns = p.parse_args()
    return EvalArgs(
        policy_path=ns.policy_path, env_id=ns.env_id, sim_backend=ns.sim_backend,
        control_mode=ns.control_mode, obs_mode=ns.obs_mode,
        object_name=ns.object_name, part_name=ns.part_name, target_letter=ns.target_letter,
        ...
        task_graph=ns.task_graph,
        results_json=ns.results_json,
    )
...
def load_policy(
    ...
    device: str,
    tokenizer_path: str | None = None,
) -> tuple[PI0Policy, PolicyProcessorPipeline, PolicyProcessorPipeline]:
    """加载 PI0 模型、preprocessor、postprocessor。

    流程与 lerobot 官方 eval 完全一致：
    1. PreTrainedConfig.from_pretrained → 读取 config.json
    2. PI0Policy.from_pretrained    → 加载模型权重
...
def _to_uint8_rgb(img: Any) -> np.ndarray:
    # Accept torch tensors directly — env.render() returns a CUDA tensor on
    # GPU sim backends, which np.asarray can't ingest. Detach + cpu + numpy
...
@torch.no_grad()
def _overlay_pi0_cam(policy: PI0Policy, image_rgb: np.ndarray, alpha: float = 0.45) -> np.ndarray:
...
def _run_task_graph_eval(args: EvalArgs, policy, preprocessor, postprocessor, task: str) -> None:
    """Rollout policy against a task-graph YAML, collecting stage flags + smoothness.

    Reuses the same per-step plumbing as the single-skill path (obs → batch →
    policy → action) but builds the env via :func:`utils.eval_setup.make_eval_env`
    so success comes from the YAML's compiled ``goal_predicate`` and per-stage
    progress comes from ``stage_predicates``. Output goes to ``results.json``
    (path overridable with --results-json).
    """
    ...
    print(f"[task-graph] Camera resolution: {cam_w}×{cam_h}")
    ...
    if args.max_episode_steps is not None:
        base_env_kwargs["max_episode_steps"] = args.max_episode_steps

    env, task_graph = make_eval_env(args, extra_env_kwargs=base_env_kwargs)
    ...
    episodes_out: list[EpisodeResult] = []
    pbar = tqdm(range(args.n_episodes), desc=f"task-graph {args.task_graph}")
    for ep_idx in pbar:
        ep_seed = (args.seed + ep_idx) if args.seed is not None else randint(0, 1_000_000)
        trial_dir = osp.join(args.record_dir, "task_graph", f"trial_{ep_idx + 1:04d}")
        os.makedirs(trial_dir, exist_ok=True)

        # We deliberately skip RecordEpisode in task-graph mode: the same env
        # is reused across episodes (env construction is heavy when the scene
        # builder reads capabilities + drops the object onto the table) and
        # re-wrapping it per episode would leak h5 file handles. The MP4 is
        # built manually from the per-step obs frames below; trajectory.h5
        # isn't needed for policy evaluation.
        wrapped = env

        obs, _info = wrapped.reset(seed=int(ep_seed))
        policy.reset()
        episode_actions: list[np.ndarray] = []
        obs_frames: list[np.ndarray] = []
        stage_reached: dict[str, bool] = {name: False for name in stage_names}
        ep_success = False
        ep_steps = 0
        ...
        for _ in range(int(target_steps)):
            ep_steps += 1
            if args.save_video:
                # env.render() returns the render camera in rgb_array mode and
                # is decoupled from obs_mode — more reliable than poking into
                # the obs dict, whose schema varies with obs_mode + wrappers.
                try:
                    frame = env.render()
                    obs_frames.append(_to_uint8_rgb(frame))
                except Exception as render_e:
                    if len(obs_frames) == 0:
                        # Surface the first failure so a misconfigured shader/
                        # render_mode doesn't silently produce empty videos.
                        print(f"[task-graph] env.render() failed: {render_e}")
            ...
            episode_actions.append(
                select_action_subspace(
                    act_np[0], control_mode=args.control_mode, include_gripper=False,
                ).copy()
            )
            obs, _r, _term, _trunc, info = wrapped.step(act_np[0])
            try:
            ...
            for name in stage_names:
                key = f"stage_{name}"
                if key in step_eval:
                    if bool(step_eval[key].item() if hasattr(step_eval[key], "item") else step_eval[key]):
                        stage_reached[name] = True
            if "success" in step_eval:
                if bool(step_eval["success"].item() if hasattr(step_eval["success"], "item") else step_eval["success"]):
                    ep_success = True

        if args.save_video and obs_frames:
            try:
                video_path = osp.join(trial_dir, f"{args.obs_video_name}.mp4")
                _save_cam_video(obs_frames, video_path, fps=30)
            except Exception:
                pass
        ...
        episodes_out.append(ep_result)
        pbar.set_postfix(success_rate=f"{np.mean([1.0 if e.success else 0.0 for e in episodes_out]):.3f}")
    ...
    out_path = args.results_json or osp.join(args.record_dir, "task_graph", "results.json")
    summary.write(out_path)
    print(f"\n{'='*48}")
    print(f"pi0 task-graph eval: {args.task_graph}")
    print(f"  episodes:     {summary.n_episodes}")
    print(f"  success_rate: {summary.success_rate:.3f}")
    if summary.stage_rates:
...
def main() -> None:
    ...
    # ---- Task-graph mode: completely separate path, bypasses DR profiles ----
    if args.task_graph is not None:
        _run_task_graph_eval(args, policy, preprocessor, postprocessor, task)
        return
    ...
    if args.target_letter is not None:
        env_kwargs["target_letter"] = args.target_letter
    if args.max_episode_steps is not None:
    ...
    for profile_name, profile_env_kwargs in profiles.items():
            ...
            for _ in range(int(target_steps)):
                ...
                if args.save_video and args.save_cam_overlay:
                    try:
                        ...
                        print(f"Warning: PI0 CAM overlay failed in ep={ep_idx}, step={len(cam_rollout_frames)}: {cam_e}")
        ...
        print(f"\n{'='*48}")
        print(f"PI0 Eval on {args.env_id} [{profile_name}]")
