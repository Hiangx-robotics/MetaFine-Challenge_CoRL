...
# Add FGManip root to path for imports
import os, sys
current_dir = os.path.dirname(os.path.abspath(__file__))
fgmanip_root = os.path.join(current_dir, '..')  # Go up one level to FGManip root
sys.path.insert(0, fgmanip_root)

import core.env  # noqa: F401 — register all MetaFine envs (incl. toggle_switch_table)
from core.env import GraspPartEnv, SlideAlongEnv, StandUpEnv, ToggleSwitchEnv, DrawTriangleEnv
...
class Args:
    ...
    sensor_size: Optional[int] = None
    """Override width and height of all sensor cameras (square). Applied via env sensor_configs."""
    base_camera_fov: Optional[float] = None
    """Override base_camera vertical FOV in radians. hand_camera is left untouched
    (nested camera-name key in sensor_configs) so wrist view is not confounded."""

    eval_require_correct_part: Optional[bool] = None
    """Override GraspPartEnv's part-identity requirement in evaluate().
    Chained tasks recorded through a YAML task graph carry their success spec
    only in the graph, not in the trajectory's env_kwargs, so a replay falls
    back to the strict grasp check and can mislabel motion-focused demos."""
...
def main(args: Args):
    ...
    if args.sensor_size is not None or args.base_camera_fov is not None:
        # ManiSkill's update_camera_configs_from_dict: flat keys apply to every
        # sensor camera; camera-name keys apply only to that camera. Keep FOV
        # scoped to base_camera so hand_camera stays at its default π/2.
        sensor_configs = dict(env_kwargs.get("sensor_configs") or {})
        if args.sensor_size is not None:
            sensor_configs["width"] = args.sensor_size
            sensor_configs["height"] = args.sensor_size
        if args.base_camera_fov is not None:
            sensor_configs["base_camera"] = {
                **sensor_configs.get("base_camera", {}),
                "fov": args.base_camera_fov,
            }
        env_kwargs["sensor_configs"] = sensor_configs
    if args.eval_require_correct_part is not None:
        env_kwargs["eval_require_correct_part"] = args.eval_require_correct_part
        ori_env_kwargs["eval_require_correct_part"] = args.eval_require_correct_part
